...
# --- Configuration ---
# Ensure imports are at the top, fixing the previous error.
st.set_page_config(
...
def get_rule_based_status(value, limit):
    ...
    # Define Warning and Critical based on the single user-defined limit
    # This example uses: Warning >= 1x limit, Critical >= 2x limit
...
# Use @st.cache_resource for the ML model (the global resource)
@st.cache_resource
def run_prophet_forecast(df, gas_col, forecast_years=5):
    ...
    # Remove any NaN or infinite values that might crash Prophet
    prophet_df.replace([np.inf, -np.inf], np.nan, inplace=True)
    prophet_df.dropna(inplace=True)
    
    if len(prophet_df) < 2:
        st.warning("Insufficient data points (less than 2) to run a trend forecast for this gas.")
        return None, None
    
    # 2. Train Prophet model
    m = Prophet(
        # Must disable seasonalities for annual data
...
if selected_gas:
    limit_value = st.session_state.ref_limits[selected_gas]
    ...
    if model and forecast_results is not None:
        ...
        # 2. Forecast Data (Prophet yhat)
        historical_end_date = df['Sampling Date'].max()
        forecast_dates = forecast_results[forecast_results['ds'] > historical_end_date]['ds']
        
        fig.add_trace(go.Scatter(x=forecast_dates, y=forecast_results[forecast_results['ds'] > historical_end_date]['yhat'], 
        ...
        st.metric(f"Current {selected_gas} Status (Latest Sample: {latest_value} ppm)", current_status)
        ...
        if limit_value > 0:
            ...
            # Find the first date where yhat meets or exceeds the critical limit
            ...
            if not forecast_critical.empty:
             ...
             st.warning("Cannot provide predictive warning as the Reference Limit is set to 0. Please set a limit to enable Rule-Based and ML analysis.")
    
    elif model is None and forecast_results is None:
        st.error("Not enough data points to run the ML forecast. Requires at least two annual samples.")
